...
if [ -d $project_work_dir/PB_prot ];
then
   rm -r $project_work_dir/PB_prot
fi
mkdir $project_work_dir/PB_prot
PB_dir=$project_work_dir/PB_prot

cd $PB_dir
echo 'performing protein blasting...'
(time /paracel/paracel/bin/pb blastall -p blastx -i $work_fasta -d nr --dbpart=1 --querypart=11000  -b 10 -v 10 -e 0.001  -m 7 -I T -o nt_unknown_prot.out 2>nt_unknown_prot.err) >& nt_unknown_prot.time
python $path_htsa_dir/viralmeta_bioifo/blast_module/parse_blastx.py nt_unknown_prot.out gi.blast_results prot.blastx_results
...
cat prot.blastx_results | awk -F"@" '{print $1}' | sort -k1,1 | awk '!x[$1]++' > prot_SEQ_ID
cat $work_fasta | awk 'BEGIN{RS=">"}NR>1{sub("\n","\t"); gsub("\n",""); print RS$0}' | awk -F"\t" '{gsub(">","",$1); print $1,$2}' | awk 'NR==FNR{a[$1];next} !($1 in a) {print ">"$1,$2}' prot_SEQ_ID - > blastx_unknown.fasta
sed -i 's/ /\n/g' blastx_unknown.fasta
############################
