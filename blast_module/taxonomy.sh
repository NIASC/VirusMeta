...
if [ -e $project_work_dir/hg19/nr_ref_hg19_unmasked.txt ]; then
   cp $project_work_dir/hg19/nr_ref_hg19_unmasked.txt $project_work_dir/NR/nr_hg_pre
elif [ -e $project_work_dir/hg19/nr_ref_hg19.txt ]; then 
   cp $project_work_dir/hg19/nr_ref_hg19.txt $project_work_dir/NR/nr_hg_pre
else
   echo "No hg19 nr file is available, Something is wrong.."
fi

cp $project_work_dir/BACTERIA/nr_ref_BACTERIA.fasta.txt $project_work_dir/NR/nr_BAC_pre
cp $project_work_dir/PHAGE/nr_ref_PHAGE.fasta.txt $project_work_dir/NR/nr_PHG_pre
cp $project_work_dir/VECTOR/nr_ref_UniVec.txt $project_work_dir/NR/nr_VEC_pre
#anotate blast output with taxonomy
#R CMD BATCH --no-save $path_htsa_dir/viralmeta_bioifo/blast_module/taxSort.R
Rscript $path_htsa_dir/viralmeta_bioifo/blast_module/taxSort.R gi_division.txt division.txt $project_work_dir/NR/nr_ref_$work_fasta.txt $project_work_dir/NR/nr_by_index.csv $project_work_dir/NR/nr_hg_pre $project_work_dir/NR/nr_BAC_pre $project_work_dir/NR/nr_PHG_pre $project_work_dir/NR/nr_VEC_pre $project_work_dir/NR/nr_unmapped.txt $project_work_dir/NR/nr_total.txt

########################################
#First select blastn clisifed contig IDs
awk -F',' '{ print $1}' $project_work_dir/PB/nt_final.csv | awk -F'"' '{if ($2!="Queryid") print $2}'  > $project_work_dir/PB/nt_final_ID.txt
#cat $project_work_dir/PB/HG $project_work_dir/PB/HUMAN_EST $project_work_dir/PB/nt_final_ID.txt >  nt_known_seq_id.txt
#cat $project_work_dir/aggregated_dir/$work_fasta | awk 'BEGIN{RS=">"}NR>1{sub("\n","\t"); gsub("\n",""); print RS$0}' | awk -F"\t" '{gsub(">","",$1); print $1}' | awk 'NR==FNR{a[$1];next} !($1 in a) {print $1}' $project_work_dir/PB/nt_final_ID.txt - > $project_work_dir/PB/nt_unknown_ID.txt
#then select yet uknown sequences IDs the NON_HG.fasta 
cat $project_work_dir/PB/NON_HG.fasta | awk 'BEGIN{RS=">"}NR>1{sub("\n","\t"); gsub("\n",""); print RS$0}' | awk -F"\t" '{gsub(">","",$1); print $1}' | awk 'NR==FNR{a[$1];next} !($1 in a) {print $1}' $project_work_dir/PB/nt_final_ID.txt - > $project_work_dir/PB/nt_unknown_ID.txt
#and now select yet uknown sequences from the original $work_fasta file
cat $project_work_dir/aggregated_dir/$work_fasta | awk 'BEGIN{RS=">"}NR>1{sub("\n","\t"); gsub("\n",""); print RS$0}' | awk -F"\t" '{gsub(">","",$1); print $1,$2}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print ">"$1,$2}' $project_work_dir/PB/nt_unknown_ID.txt - > $project_work_dir/PB/nt_unknown.fasta
#breack lines to separate ID and corresponding sequence 
sed -i 's/ /\n/g' $project_work_dir/PB/nt_unknown.fasta
#select yet uknown >1000 bp length sequences from the original $work_fasta file
cat $project_work_dir/aggregated_dir/$work_fasta | awk 'BEGIN{RS=">"}NR>1{sub("\n","\t"); gsub("\n",""); print RS$0}' | awk -F"\t" '{gsub(">","",$1); print $1,$2}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print ">"$1,$2}' $project_work_dir/PB/nt_unknown_ID.txt - | awk '{ if(length($2)>=1000) print $1,$2}' > $project_work_dir/PB/nt_unknown_1000.fasta
sed -i 's/ /\n/g' $project_work_dir/PB/nt_unknown_1000.fasta

#########################
#Number of unmapped reads
cp $project_work_dir/NR/nr_unmapped.txt $project_work_dir/PB/nr_unmapped.txt
#Number of reads of contigs classified as unknown
cat $project_work_dir/NR/nr_ref_$work_fasta.txt |  awk 'NR==FNR{a[$1];next} !($1 in a) {print $1,2}' $project_work_dir/PB/nt_unknown_ID.txt - | awk '{ sum+=$2} END {print sum}' > nr_unknown.txt
