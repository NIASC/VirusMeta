...
if ($paired){
   ...
   if(!-e 'virus-consensus-seq.fa'){
        ...
        if (!-e $corrected_ref_file || !-s $corrected_ref_file){
            ...
            #FLAG_MARK: here I added a script to conduct preprocessing step1 and make allignment file
            if (!-e $align_file){
                my $preprocess_script         = "$script_dir/preprocess.pl";
                print "Can't find BAM file $align_file! Therefor I will start with preprocessing...\n";
...
if (!-e '$virus_sequence.bwt'){
    ...
    system("$samtools_bin faidx $virus_sequence 2> /dev/null");
}
...
if ($markdup eq 'y'){
   ...
   if (!-e 'alignment.sorted.md.bam'){
       if ($paired){
           `$samtools_bin rmdup    alignment.sorted.bam alignment.sorted.md.bam`;
       }else{
           `$samtools_bin rmdup -s alignment.sorted.bam alignment.sorted.md.bam`;
       }
       `$samtools_bin index alignment.sorted.md.bam`;
   }
