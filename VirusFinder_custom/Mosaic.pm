...
sub GetInsertSize{
    ...
    my @insert_stat;
    my $recordcounter=0;
    #FLAG_MARK: here I increase 50000 to 100000000
    my $expected_max=100000000;
    ...
    #FLAG_MARK:
    print "current values: $stdm $nstdm $stdp $nstdp\n";
    if ($stdm==0 || $nstdm==0 || $stdp==0 || $nstdp==0){
       print "Etimating my own insertsize distribution from $BamFile...\n";
       my $instersize_results = `samtools view $BamFile | python /media/StorageOne/HTS/viralmeta_bioifo/VirusFinder_custom/mean_size.py`;
       my @results_tab = split(/\s+/, "$instersize_results");
       my $lower = $results_tab[0];
       my $upper = $results_tab[1];
       my $mean  = $results_tab[2];
       my $std   = $results_tab[3];
