...
if (!-e "$output_dir/unmapped.1.fa" || ($paired && !-e "$output_dir/unmapped.2.fa")){
    ...
    if ($paired){
        `$samtools_bin view -u -f 4  -F 264 $output_dir/alignment.bam -o $output_dir/L.unmapped.bam`;
        `$samtools_bin view -u -f 8  -F 260 $output_dir/alignment.bam -o $output_dir/R.unmapped.bam`;
        `$samtools_bin view -u -f 12 -F 256 $output_dir/alignment.bam -o $output_dir/B.unmapped.bam`;
        `$samtools_bin merge -u - $output_dir/[LRB].unmapped.bam | $samtools_bin sort -n - $output_dir/unmapped`;
        ...
        `$samtools_bin view -uf 4 $output_dir/alignment.bam > $output_dir/unmapped.bam`;
...
sub PreprocessBam {
    ...
    $paired = `$samtools_bin view $alignment_file | head -10 | awk '{if(and(\$2,1)>0)print}' | wc -l`;
    chomp $paired;
}
...
sub PreprocessFastq {
    ...
    `$samtools_bin view -bS $output_dir/alignment.sam -o $alignment_file`;
