...
if (!-e 'alignment.sorted.bam'){
   ...
   `$samtools_bin view -bS alignment.sam -o alignment.bam 2> /dev/null`;
   `$samtools_bin sort   alignment.bam   alignment.sorted`;
   `$samtools_bin index  alignment.sorted.bam`;
...
sub RunSVDetect
{
    ...
    if ($config->has_value("alignment_file") && !-e 'alignment+.bam') {
        ...
        if ($count > 0){
          ...
          `cat unmapped.2.fq >> unmapped.2+.fq`;
           ...
           system("$bwa_bin aln -t $thread_no hg19+virus.fa $unmappedfq2 > unmapped.2.sai 2> /dev/null");
           system("$bwa_bin sampe -r '$RG' hg19+virus.fa unmapped.1.sai unmapped.2.sai $unmappedfq1 $unmappedfq2 > alignment+.sam 2> /dev/null");
           `$samtools_bin view -bS alignment+.sam -o alignment+.bam`;
           `rm alignment+.sam`;
        }
    }
    ...
    if (!-e 'SVDetect/hg19+virus.len'){
        my @chr_len = `$samtools_bin view -H alignment.sorted.bam | awk '/\@SQ/'`;
    ...
    #FLAG_MARK: here I decreased nb_pairs_threshold from 3 to 1. Check what it is
    #print OUT "nb_pairs_threshold=3\n";
    print OUT "nb_pairs_threshold=1\n";
    ...
    if (-e 'alignment+.bam'){
        if (!-e "$output_dir/SVDetect/alignment+.ab.bam.inter.links"){
    	    ...
    	    `awk '{if(match(\$0,/chrVirus/) && \$13>=0.9)print}' SVDetect/alignment+.ab.bam.inter.links.filtered.sv.txt > SVDetect/results-virus-loci.txt`;
             `cp SVDetect/results-virus-loci.txt SVDetect/bla_a_results-virus-loci.txt`;
        }else{
            print "\nWarning: SVDetect did find virus integration event!\n\n";
        }
    }else{
        if (!-e "$output_dir/SVDetect/alignment.sorted.ab.bam.inter.links"){
            ...
            `cp SVDetect/results-virus-loci.txt SVDetect/bla_b_results-virus-loci.txt`;
        }else{
            print "\nWarning: SVDetect did find virus integration event!\n\n";
        }
    }
...
sub GetSoftClippedReads {
    ...
    my $sFILe      = "$samtools_bin view $align_file | awk '$awk_cmd' |";
    my %sReads     = ();
    ...
    my $count = 0;
    $sFILe = "$samtools_bin view $align_file |";
    open (IN, $sFILe);
