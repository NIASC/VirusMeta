...
sub RunSensitiveMode {
    ...
    print "Etimating my own insertsize distribution from $align_file...\n";
    my $instersize_results = `samtools view $align_file | python /media/StorageOne/HTS/viralmeta_bioifo/VirusFinder_custom/mean_size.py`;
    my @results_tab = split(/\s+/, "$instersize_results");
    my $g_lower = $results_tab[0];
    my $g_upper = $results_tab[1];
    my $g_mean  = $results_tab[2];
    my $g_std   = $results_tab[3];
    ...
    my $corrected_ref_file = sprintf "refseq.fa.%d", $sensitivity_level;
