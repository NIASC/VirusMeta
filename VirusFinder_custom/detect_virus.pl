...
#FLAG_MARK: here I added process parameters. If provided will skip trinity assembly
# process parameters
my $detect_virus = $config->get_value("detect_virus");
my $potenial_viral_db = $config->get_value("potenial_viral_db");
...
my $wrapfasta_bin      = $config->get_value("wrapfasta_bin");
my $blast_xml_parser_bin = $config->get_value("blast_xml_parser_bin");
...
my $sort_top_virus_bin    = $config->get_value("sort_top_virus_bin");
my $select_top_virus_bin  = $config->get_value("select_top_virus_bin");

my $index_name            = $config->get_value("index_name");
my $virus_index_file      = $config->get_value("virus_index_file");
...
#FLAG_MARK: here I say to the pipeline to skip blat_out_candidate
chomp($detect_virus);
if ( $detect_virus eq 'yes' ){
   ...
   if (!-e 'blat_out_candidate_singlelane.fa') {
      ...
      #FLAG_MARK: here I added to check file size
      my $blat_out_size = -s 'blat_out_candidate_singlelane.fa';
      if ( $blat_out_size < 108 ) {
         print "blat_out_candidate_singlelane.fa is very small!! I will use singlelane_unmapped.fa instead...\n ";
         `cp singlelane_unmapped.fa  blat_out_candidate_singlelane.fa`;
      }else{
         `rm  chopped_singlelane.psl singlelane_unmapped.fa`;
      }
   }
      ...
      print "skipping blat_out_candidate...\n ";
      #`touch  blat_out_candidate_singlelane.fa`;  
}
...
if (!-e 'Trinity-init.fasta'){
  $updated = 1;
  #FLAG_MARK: here I say to the pipeline weather I want it or not to detect viruses  
  chomp($detect_virus);
  if ( $detect_virus eq 'yes' ){  
    my $ILIBs = GetIdir();
    ...
    #`perl $ILIBs $trinity_script --seqType fa --max_memory 4G --single blat_out_candidate_singlelane.fa --min_contig_length $min_contig_length  --output trinity_output --CPU $thread_no --bfly_opts \"-V 10 --stderr\"`;
    #`perl $ILIBs $trinity_script --seqType fa --max_memory 4G --single blat_out_candidate_singlelane.fa --min_contig_length $min_contig_length  --output trinity_output --CPU $thread_no`;
    if (-e 'trinity_output/Trinity.fasta' && -s 'trinity_output/Trinity.fasta'){
        ...
        if (!-e 'trinity_output'){
            ...
            #`perl $ILIBs $trinity_script --seqType fa --max_memory 4G --single blat_out_candidate_singlelane.fa --min_contig_length $contig_len  --output trinity_output --CPU $thread_no --bfly_opts \"-V 10 --stderr\"`;
             #`perl $ILIBs $trinity_script --seqType fa --max_memory 4G --single blat_out_candidate_singlelane.fa --min_contig_length $contig_len  --output trinity_output --CPU $thread_no`;
	    if (!-e 'trinity_output/Trinity.fasta' || !-s 'trinity_output/Trinity.fasta'){
            	`rm -r trinity_output`;
    ...
    print "Coping predetermined potentially viral sequences $potenial_viral_db ...\n";
    #`cp $potenial_viral_db Trinity-init.fasta`; 
    `$wrapfasta_bin $potenial_viral_db Trinity-init.fasta`;
...
if ($updated || !-e 'singlelane.psl' || !-e 'non_human_contig_blastn.txt'){
   $updated = 1;
   #FLAG_MARK: here I will say to the pipeline weather whic´n blastn sortig algorithm to use
   chomp($detect_virus);
   if ( $detect_virus eq 'yes' ){
    ...
    if ($usethreads && $thread_no > 1){
        ...
        `$blat_bin non_human_contig.fa -minIdentity=98 blat_out_candidate_singlelane.fa singlelane.psl`;
         ...
         print "Potentially viral sequences are provided so I will use our own blasn sorting algorith and skip the step 2.5 map reads to contigs...\n\n";
         `$blastn_bin -db $blastn_index_virus -query $potenial_viral_db -word_size 11 -gapopen 0 -gapextend 2 -penalty -1  -reward 1 -evalue $blastn_evalue_thrd -show_gis -outfmt 5 -num_threads 11 > non_human_contig_blastn.xml`;
         `python $blast_xml_parser_bin --input_file=non_human_contig_blastn.xml --result_file=final.blast_results --out_gi=gi.blast_results  --temp_directory=pbNT_tmp --jobs=70  >/dev/null 2>&1`;
         `rm -rf pbNT_tmp`;
         `Rscript $sort_top_virus_bin final.blast_results $index_name $virus_index_file non_human_contig_blastn.txt top_virus_id.txt results-virus_pre.txt`;
   }
}
...
if ( $detect_virus eq 'yes' ){
   ...
   if ($updated || !-e 'results-virus-top1.fa' || !-s 'results-virus-top1.fa' || !-e 'results-virus.txt' || !-s 'results-virus.txt' || !-e 'results-virus-list.txt' || !-s 'results-virus-list.txt'){  
      `$select_top_virus_bin $blastn_index_virus top_virus_id.txt results-virus-top1.fa results-virus_pre.txt results-virus.txt results-virus-list.txt results-contig.txt`;
   }
}
chdir $start_dir;
