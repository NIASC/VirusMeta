...
#/media/StorageOne/HTS/viralmeta_bioifo/ffp/ffp_step1.sh /media/StorageOne/HTS/PublicData/nt_pb/virus_block_ffp_7 /media/StorageOne/HTS/PublicData/nt_pb/family 7
...
do
        #echo "$FILE"
        python $path_htsa_dir/$path_pipeline/ffp/ffp_prep.py $FILE
done
...
awk -v working_dir=$working_dir '{print "mv  "working_dir"/"$1"-*.ffp "$1".ffp"}' genomes.txt > correct_ffp_names.sh
chmod +x ./correct_ffp_names.sh
#and execute it
./correct_ffp_names.sh
