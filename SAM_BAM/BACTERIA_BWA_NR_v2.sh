...
/usr/local/bin/bwa mem $db $PAIR1 $PAIR2 -t 40 -M | /usr/local/bin/samtools view  -@ 40 -b -F 4 - | /usr/local/bin/samtools view -@ 40 -  | awk '{ print $1 }' > BAC_ID
...
gzip -dc $PAIR1 | paste - - - - | awk -F" " '{gsub("@","",$1); print $1,$2,$3,$4}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print "@"$1,$2,$3,$4}' $BACTERIA/NON_BAC_ID - | tr ' ' '\n' | gzip > $BACTERIA/NON_HGBAC_read1.fastq.gz & gzip -dc $PAIR2 | paste - - - - | awk -F" " '{gsub("@","",$1); print $1,$2,$3,$4}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print "@"$1,$2,$3,$4}' $BACTERIA/NON_BAC_ID - | tr ' ' '\n' | gzip > $BACTERIA/NON_HGBAC_read2.fastq.gz
#Remove files
