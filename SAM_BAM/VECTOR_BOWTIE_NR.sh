...
mkdir $VECTOR
db=$path_htsa_dir/PublicData/UniVec/bowtie/UniVec
...
cat $PAIR1 | paste - - - - | awk -F" " '{gsub("@","",$1); print $1,$2,$3,$4}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print "@"$1,$2,$3,$4}' $VECTOR/NON_VEC_ID - | tr ' ' '\n' > $VECTOR/NON_HGBACPHGVEC_read1.fastq
cat $PAIR2 | paste - - - - | awk -F" " '{gsub("@","",$1); print $1,$2,$3,$4}' | awk 'NR==FNR{a[$1];next} ($1 in a) {print "@"$1,$2,$3,$4}' $VECTOR/NON_VEC_ID - | tr ' ' '\n' > $VECTOR/NON_HGBACPHGVEC_read2.fastq
#Sort file
...
if [ -f "$project_work_dir/Data/Intensities/BaseCalls/forward_index_name.txt" ]; then 
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/hg19/HG19_ID  $project_work_dir/Data/Intensities/BaseCalls/forward_index_name.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/hg19/nr_HG19_by_index
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/BACTERIA/BAC_ID $project_work_dir/Data/Intensities/BaseCalls/forward_index_name.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/BACTERIA/nr_BAC_ID_by_index
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/PHAGE/PHG_ID $project_work_dir/Data/Intensities/BaseCalls/forward_index_name.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/PHAGE/nr_PHG_ID_by_index
   ...
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/hg19/HG19_ID  $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/hg19/nr_HG19_by_index
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/BACTERIA/BAC_ID $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/BACTERIA/nr_BAC_ID_by_index   
   awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/PHAGE/PHG_ID $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/PHAGE/nr_PHG_ID_by_index
   ...
   gzip -dc $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt.gz | awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/hg19/HG19_ID - | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/hg19/nr_HG19_by_index
   gzip -dc $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt.gz | awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/BACTERIA/BAC_ID - | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/BACTERIA/nr_BAC_ID_by_index
   gzip -dc $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt.gz | awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $project_work_dir/PHAGE/PHG_ID - | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $project_work_dir/PHAGE/nr_PHG_ID_by_index
   gzip -dc $project_work_dir/Data/Intensities/BaseCalls/forward_index_name_sorted.txt.gz | awk 'NR==FNR{hash[$1];next} ($2 in hash) {print $1,$2}' $VECTOR/VEC_ID - | awk '{arr[$1]++} END {for(i in arr) print i,arr[i]}' >  $VECTOR/nr_VEC_ID_by_index
fi

#Compress fastq files
gzip $VECTOR/NON_HGBACPHGVEC_read1.fastq
gzip $VECTOR/NON_HGBACPHGVEC_read2.fastq
#######################################################
...
rm $project_work_dir/hg19/NON_HG19_read1.fastq
rm $project_work_dir/hg19/NON_HG19_read2.fastq
rm $project_work_dir/BACTERIA/NON_HGBAC_read1.fastq
rm $project_work_dir/BACTERIA/NON_HGBAC_read2.fastq
rm $project_work_dir/PHAGE/NON_HGBACPHG_read1.fastq
rm $project_work_dir/PHAGE/NON_HGBACPHG_read2.fastq
