...
def sam_linkage_claster (linkage_info, output):
   """
    sam_linkage_claster takes 2 arguments:
    1) linkage_info file which was generated from sam file.
       it has 3 tab seperated columns: read_id, seq_id(link1), seq_id(link2)
   ...
   with open(linkage_info,"r") as self_linkage_info, open(output,"w") as self_outp_result:
     for lines in self_linkage_info:
         ...
         read_id, link1, link2 = lines.strip().split('\t')
         link1 = link1.strip()
         link2 = link2.strip()
