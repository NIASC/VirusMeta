...
/usr/local/bin/bwa mem $work_fasta $PAIR1 $PAIR2 -t 40 -M | /usr/local/bin/samtools view -@ 40 -b -S - > aln-pe.bam
...
cat sam_final_$work_fasta.txt | awk -F"\t" '{if($12 == 0) print $1}' | sort -k1,1 -T $1 | awk '!x[$1]++' > $work_fasta.ID
cat sam_final_$work_fasta.txt | awk -F"\t" '{if($12 == 0) print $1,$3}' > $work_fasta.seq
...
#TODO: too slow
#python /media/storage/HTS/viralmeta_bioifo/SAM_BAM/sam_linkage_claster.py Linkage_info_v2.txt cluter_by_sam.txt 
###################################  
