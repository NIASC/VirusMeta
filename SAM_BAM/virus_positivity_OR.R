#Rscript /media/StorageOne/HTS/viralmeta_bioifo/SAM_BAM/virus_positivity_OR.R '/media/StorageOne/HTS/viralmeta_bioifo' '/media/StorageOne/HTS/Projects/2011_G5_96ALL-HiSeq' '/media/StorageOne/HTS/Projects/2011_G5_96ALL-HiSeq/PB/virus_final_index.csv' 'Family' '/media/StorageOne/HTS/Projects/2011_G5_96ALL-HiSeq/PB/virus_taxa_OR.csv' 5 47 47 0 '/media/StorageOne/HTS/Projects/2011_G5_96ALL-HiSeq/case_control_id.txt'

args<-commandArgs(TRUE)
path_pipeline = sprintf("%s", args[1])
#path_to_project = sprintf("%s", args[2])
virus_by_index_file = sprintf("%s", args[2])
clustering_factor  = sprintf("%s", args[3])
write_file_loc = sprintf("%s", args[4])
number_of_cases = as.numeric(sprintf("%s", args[5]))
number_of_controls  = as.numeric(sprintf("%s", args[6]))
positivity_cuttoff  = as.numeric(sprintf("%s", args[7]))
number_of_blank = as.numeric(sprintf("%s", args[8]))
case_control_id_file = sprintf("%s", args[9])
...
colnames(case_control_id)<-c("Sample_id","Case_CTRL")
    ...
    #print (table(VIR_NR$Division_virus))
    ###-------------------
    ...
    #TODO: Superkingdom_virus need coorection in a code
    ...
    if (clustering_factor== "Family"){
       ...
       NR<-merge(NR,total_viruses,all=T)
       rownames(NR)<-NR$Family
       #write.csv(NR, "/media/StorageOne/HTS/Projects/2015_F_Lagheden_ACCES_HPVneg/PB/vir_nr_by_family.csv", row.names=F)  
    }
    ...
    ###########################################
    ###If there are cases and controls
    ###Calculate OR and number of positive numbers
    if ( number_of_cases >= 1 ) {
    ###!!!!!!!!!!!!!!!!!!!!!!!
    ##########################
    #if we have blank samples 
    #blank names should blank1,blank2,...
    #print (table(rownames(pos)))
    if (number_of_blank > 0){   
       if (clustering_factor != "Division_virus" & clustering_factor != "Superkingdom_virus" & clustering_factor != "Family"){
    ...
    #If there was more than 2 samples names that start with number
    ...
    if (iter_item > 2){
        ...
        case_label<-selected_cases$Sample_id #Sample_name
        ctrl_label<-selected_ctrl$Sample_id #Sample_name
    }
print ("**************")
...
number_of_controls <- length (ctrl_label)
 ...
 print ("**************")   
    ...
    #Otherwise calculate number of positive numbers
    }else{
        ##!!!!!!!
        #here I need a list of samples to remove undetermined and negative
        ##!!!!!!!
        pos$POS_total<-rowSums(pos)
        NR$NR_total<-rowSums(NR)
        pos$Cluster<-rownames(pos)
        NR$Cluster<-rownames(NR)
        pos<-pos[,c("Cluster","POS_total")]
        NR<-NR[,c("Cluster","NR_total")]
        final<-merge(pos,NR,all=T)
    }
